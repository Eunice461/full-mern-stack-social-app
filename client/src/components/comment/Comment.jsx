...
const handleComment = async ()=>{
    ...
    try{
        await axiosPrivate.post("/comment/posts/"+ path + "/comment", newComment, { withCredentials: true,
...
const handleCommentUpdate = async (id) =>{
    try{
          dispatch({type:"CURSOR_NOT_ALLOWED_START"});  
            await axiosPrivate.patch("/comment/posts/"+ path + "/comment/" + id, {
             author: logUser.userId,
             role: logUser.role, 
             comment:  commentdescription2,
...
const  handleCommentDelete = async (id) =>{
    try{
         dispatch({type:"CURSOR_NOT_ALLOWED_START"}); 
        await axiosPrivate.delete(`/comment/posts/${path}/comment/${id}`, {data: {author: logUser.userId, role: logUser.role}}, { withCredentials: true,
            headers:{authorization: `Bearer ${auth}`}
           })
           dispatch({type:"CURSOR_NOT_ALLOWED_START_END"});
           setCommentState(!commentState);
...
const handleReply = async (id) =>{
    ...
    try{
        dispatch({type:"CURSOR_NOT_ALLOWED_START"}); 
       await axiosPrivate.post(`/reply/posts/${path}/comments/${id}/reply`, newReply, { withCredentials: true,
            headers:{authorization: `Bearer ${auth}`}});
            dispatch({type:"CURSOR_NOT_ALLOWED_START"}); 
            setShowReply(true)
...
const handleReplyUpdate = async (id) =>{
    try{
         dispatch({type:"CURSOR_NOT_ALLOWED_START"});
        await axiosPrivate.patch(`/reply/${id}`, {
             author: logUser.userId,
             role: logUser.role, 
...
const handleDeleteComment = async (id, replyId)=>{
     try{
            dispatch({type:"CURSOR_NOT_ALLOWED_START"});
                await axiosPrivate.delete(`/reply/posts/${path}/comments/${id}/reply/${replyId}`, {data: {author: logUser.userId, role: logUser.role}},
                { withCredentials: true,
                headers:{authorization: `Bearer ${auth}`}}
                );
                //window.location.reload()
        ...
        <div className="comment">
            ...
            <h5 className='users-comment'>Users' comments</h5>
                {comments.map((singleComment, index)=>{
                    const {author, comment, _id, createdAt, replies} = singleComment
                                    ...
                                    <div className="comment-author-div">
                                        ...
                                        </div>
                                       {
                                           ...
                                           <div className="comment-edit-delete-div">
                                             <i className="singlePostIcon fas fa-edit" onClick={() => {setUpdateComment(_id); setCommentDescription2(comment)}}  ></i>
